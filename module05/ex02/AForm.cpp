...
{
    std::cout << GREEN << "AForm default constructor called" << std::endl << RESET;
}
...
AForm::AForm(std::string name, int signGrade, int execGrade) : name(name), signGrade(signGrade), execGrade(execGrade)
{
    ...
    std::cout << GREEN  << "AForm " << this->name << " constructor called" << std::endl << RESET;
}

AForm::~AForm(void)
{
    std::cout << RED << "AForm " << this->name << " destructor called" << std::endl << RESET;
}   
...
{
    try {
        AForm *Aform = new AForm(name, signGrade, execGrade);
        return (Aform);
    } catch (const std::exception &e) {
        std::cout << RED << "Error creating Aform: " << e.what() << std::endl << RESET;
        return (NULL);
    }
}

void deleteAForm(AForm *Aform)
{
    if (Aform == NULL)
        return;
    delete Aform;
...
form::AForm(std::string name, int signGrade, int execGrade) : name(name), signGrade(signGrade), execGrade(execGrade)
{
    this->signedStatus = false;
    try
    {
